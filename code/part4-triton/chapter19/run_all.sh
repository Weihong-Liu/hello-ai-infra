#!/usr/bin/env bash
# run_all.sh — Triton Softmax 章节实验入口
#
# 假设调用前已经执行：
#   cd code/part4-triton
#   source ./activate-rocm.sh
#
# 所有输出写到 chapter3/logs/
# 失败时返回非零退出码。
...
echo "=== Triton Softmax 实验 ===" | tee "$LOG_DIR/run_all.log"
...
# --- 步骤 1：数值验证 ---
echo "[ 步骤 1 ] 数值验证 vs torch.nn.functional.softmax ..."
python verify_vs_torch.py 2>&1 | tee "$LOG_DIR/verify_vs_torch.log"
echo "数值验证完成" | tee -a "$LOG_DIR/run_all.log"
echo ""

# --- 步骤 2：Benchmark ---
echo "[ 步骤 2 ] Benchmark（warmup=25, repeat=100）..."
python bench_softmax.py --warmup 25 --repeat 100 2>&1 | tee "$LOG_DIR/bench_full.log"
