---
title: "第6章 第一个 AMD GPU 程序与 baseline"
description: "Hello GPU 第6章 · PyTorch ROCm + 最小 HIP kernel 双路径，建立可复用的 benchmark 习惯"
---

# 第6章 第一个 AMD GPU 程序与 baseline
...
## 6.1 从已经验证的 ROCm 环境开始

在写第一行 GPU 代码之前，有一件事必须确认：环境是通的。好在这件事[第 1 章](../../part0-preface/chapter1/index.md)已经帮你做完了——三道环境验证门（`rocminfo` 能看到 GPU、PyTorch ROCm 能跑 GPU tensor、最小 HIP 程序能编译运行）都已经通过。如果你还没做，请先回去跑完那三道门。本章的 `uv sync` 和 `hipcc` 都建立在那一章的基础上，跳过这一步很容易卡在第一行命令上。
...
如果这里无法激活环境，先回到[第 1 章环境准备与验证](../../part0-preface/chapter1/index.md)排查 `uv` 环境、ROCm wheel 和 `_rocm_sdk_devel` 初始化问题。

## 6.2 跑通第一个 HIP kernel
...
如 @fig-vec-add-data-path 所示，真正的加法只发生在 kernel 里，但一个完整 GPU 程序还必须处理显存分配、数据拷贝和结果校验。

完整 Host 端流程在[第 1 章 1.5 节](../../part0-preface/chapter1/index.md#_1-5-验证最小-hip-程序)已经作为环境验证出现过。为了方便对照，这里再把完整文件折叠贴一次：
...
外面那一大段 `main()` 在做的事，其实就是 @fig-vec-add-data-path 那条数据路径：分配显存 → 拷数据 → 启动 kernel → 拷回结果 → 校验 → 释放。这一层在第 1 章已经讲过，我们这一章不重复。**真正值得重点看的是中间那段 kernel**，它才是跑在 GPU 上的代码：
...
如 @fig-block-thread-hierarchy 所示，Grid 是 4096 个 Block 串成的大链；放大其中任意一个 Block，里面是 256 个并行 Thread；每个 Thread 通过 `blockIdx.x × blockDim.x + threadIdx.x` 算出自己在整条数据数组里负责哪一格。
...
`max_error: 0` 表示每个输出元素都等于期望值 `3.0`。输出里的 `blocks: 4096`、`threads_per_block: 256` 也不神秘：程序里 `n = 1 << 20 = 1048576`，每个 block 使用 256 个线程，所以 `1048576 / 256 = 4096` 组。换句话说，**我们刚才同时叫起了 1048576 个 GPU 工人**，每人负责数组里的一个元素，齐刷刷做了一次加法（如 @fig-one-thread-one-element 所示）。

::: figure fig-one-thread-one-element
![一线程一元素](./images/one-thread-one-element.png)

一线程一元素：每个 GPU 线程负责一个数据元素
:::

这一步通过后，你已经跑通了第一段真正由自己编译的 AMD GPU kernel——欢迎正式进入 GPU 编程的世界。

## 6.3 建立 baseline benchmark
...
warmup: 5
repeat: 30
cpu_mean_ms: 6.859917
cpu_median_ms: 5.314185
cpu_min_ms: 4.869285
cpu_bandwidth_gb_s_by_min: 41.346233
gpu_mean_ms: 0.879170
gpu_median_ms: 0.879930
gpu_min_ms: 0.839396
gpu_bandwidth_gb_s_by_min: 239.846976
status: PASS
```
...
按这个口径，本次 AI MAX 395 + ROCm 7.12.0 上的 CPU baseline 约为 **41.3 GB/s @ AI MAX 395, fp32**，GPU baseline 约为 **240 GB/s @ AI MAX 395, fp32**。GPU 约是 CPU 的 `240 / 41.3 ≈ 5.80` 倍，可以先粗略记成 6 倍左右。

这个差距不神秘：Vector Add 主要是在搬数据，GPU 侧有更适合大规模连续读写的并行执行和内存访问路径。不过这个数字不是硬件理论带宽，也不是后续优化目标。

作为粗略参照：AI MAX 395 公开规格上的统一内存带宽约 256 GB/s 级别。我们这次跑出来的 GPU baseline 约 240 GB/s，看起来已经很接近这个数字——但要小心：256 GB/s **不是本教程实测**出来的理论上限，240 GB/s 也只是 PyTorch elementwise benchmark 下的有效带宽估算。后面 profiling 篇会用更严格的方法把 kernel 时间、数据搬运、缓存行为和测量误差一一拆开。

想查自己机器的理论带宽，最稳妥的方式是去 AMD 官方规格表确认（见本章延伸阅读）。无论查到的数字是多少，请记住一件事：**理论带宽是上限，不是承诺**——任何 benchmark 跑出来的有效带宽，都只能逼近、不会超过它。

## 6.4 留下实验记录

跑完前面三段命令，你大概觉得事情已经做完了——其实还没有。**性能工作真正麻烦的一刻，往往不是第一次没跑快，而是过几天回头看时，你自己也说不清当时跑了哪个版本、用了什么输入、那个 240 GB/s 到底是怎么量出来的。** 没留记录的实验，三天后基本等于白做。
...
cd code/part1-hardware-rocm
source ./activate-rocm.sh
python chapter5/check_torch_rocm.py
cd chapter3
